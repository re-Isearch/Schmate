...
Data Generation: Creates normalized random vectors with configurable dimensions and count
Ground Truth Computation: Calculates exact k-NN using brute-force L2 distance
Benchmarks 9 Configurations:

BIN1: STANDARD, BETTER, CENTROID
INT158: STANDARD, CENTROID
INT4: STANDARD, CENTROID
INT8: STANDARD, CENTROID


Metrics Measured:

Build time (index construction)
Query time (for 1000 queries)
Recall@k (accuracy)
Bytes per vector (memory efficiency)
Total memory usage


Incremental Training: Uses add_to_centroid() during index construction for CENTROID modes
...
// Compute ground truth using exact L2 distance
vector<vector<size_t>> compute_ground_truth(
    ...
    for (size_t q = 0; q < queries.size(); ++q) {
        ...
        for (size_t i = 0; i < data.size(); ++i) {
            ...
            for (size_t d = 0; d < data[i].size(); ++d) {
                float diff = data[i][d] - queries[q][d];
                dist += diff * diff;
            }
...
BenchmarkResult run_benchmark(
    ...
    for (size_t i = 0; i < n; ++i) {
        // Add to centroid for incremental training
        space->add_to_centroid(data[i].data());
...
int main() {
    ...
    auto ground_truth = compute_ground_truth(data, queries, k);
    cout << endl;
