...
NOTE: This version is a FORK of the version 0.8.0 HNSWlib C++ code primarily for
use in re-Isearch/Schmate.

**NEWS:**

**version 1.1/IB **
SVE support:
Key SVE Intrinsics Used:
svcntw()                    // Get vector length (float32 elements)
svptrue_b32()              // Create all-true predicate
svwhilelt_b32(i, qty)      // Create predicate for remaining elements
svld1_f32(pred, ptr)       // Load floats with predication
svst1_f32(pred, ptr, vec)  // Store floats with predication
svsub_f32_z(pred, a, b)    // Subtract with zeroing
svmla_f32_z(pred, a, b, c) // Multiply-accumulate (a + b*c)
svaddv_f32(pred, vec)      // Horizontal reduction (sum)

Expected Performance Gains:
Compared to NEON (128-bit):

256-bit SVE: 1.5-1.6x faster
512-bit SVE: 2.0-2.5x faster

Expected Performance ranking: SVE > AVX512 > AVX > NEON > Scalar

**version 1.0/IB Schmate Fork**
